...
export const withTransactionData = (req: TransactionDataRequest) => {
	const TransactionDataComponent: React.FC = () => {
		...
		if (req.type === 'urn:wwwallet:example_transaction_data_type') {
			return (
				<div className="p-2 text-c-lm-gray-800 text-sm dark:text-c-dm-gray-200 mt-2">
			...
			return (
				<div className="pd-2 text-c-lm-gray-800 text-sm dark:text-c-dm-gray-200 mt-2 mb-2">
					<span className="text-primary text-sm font-bold dark:text-white block mb-1">
						{t('selectCredentialPopup.signedDocuments')}
					</span>
					<p></p>
					<div className="flex w-full border rounded-md dark:border-c-dm-gray-400 bg-c-lm-gray-100 dark:bg-c-dm-gray-900 p-2">
						<div className="w-5/6 flex items-center">
							<ul className="flex flex-col list-disc">
								{req.documentDigests.map((digest, index) => (
									<span className="text-c-lm-gray-800 dark:text-c-dm-gray-200 font-bold">
										{digest.label}
									</span>
								))}
							</ul>
