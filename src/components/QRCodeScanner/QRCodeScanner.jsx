...
const QRCodeScanner = ({ onClose }) => {
	...
	const getDevices = () => {
		...
		.then(async mediaDevices => {
			...
			for (const device of videoDevices) {
				...
				resolution.idealHeight = Math.min(resolution.height, resolution.width, 1080);
				...
				if (isBackCamera && (!bestBackCamera || bestBackCamera.resolution.width * bestBackCamera.resolution.height < resolution.width * resolution.height)) {
					bestBackCamera = { device, resolution, facingMode: 'environment' };
					...
					bestFrontCamera = { device, resolution, facingMode: 'user' };
				}
	...
	return (
		<PopupLayout isOpen={true} onClose={handleClose} loading={loading} fullScreen={screenType !== 'desktop'}>
			{ (!hasCameraPermission || cameraReady) && (
				<>
					{ cameraReady && screenType === 'mobile' ? (
						<div className='flex'>
