...
const NavItem = ({
	...
	return (
		<button
			...
			className={`cursor-pointer flex items-center space-x-2 mb-4 p-2 rounded-r-xl w-full ${location.pathname === path ? 'bg-white text-custom-blue' : 'nav-item-animate-hover'}`}
		>
			{children}
		</button>
	);
