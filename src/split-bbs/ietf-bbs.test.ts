...
import { fromHex, I2OSP, toHex, toU8 } from "../util";
import { getCipherSuite, make_create_generators, make_hash_to_scalar, makeKeyGen, messages_to_scalars } from "./ietf-bbs";
import { PointG1 } from ".";
...
describe("hash_to_scalar", () => {
	it("passes test vectors", async () => {
		...
		const suite = getCipherSuite('BBS_BLS12381G1_XMD:SHA-256_SSWU_RO_', dst);
...
describe("keyGen", () => {
	it("passes test vectors", async () => {
		...
		const SK = await makeKeyGen(suite)(key_material, key_info, new TextEncoder().encode("BBS_BLS12381G1_XMD:SHA-256_SSWU_RO_H2G_HM2S_" + "KEYGEN_DST_"));
...
describe("messages_to_scalars", () => {
	it("passes test vectors", async () => {
		...
		const scalars = await messages_to_scalars(messages, api_id, suite);
...
describe("create_generators", () => {
	it("passes test vectors", async () => {
		const functionSuite = hashToCurve("BLS12381G1_XMD:SHA-256_SSWU_RO_" as SuiteId, new TextEncoder().encode("irrelevant, this is not used in expand_message"));

		const count = 11;
		const create_generators = make_create_generators({
			hash_to_curve_g1: (msg: BufferSource, DST: BufferSource) =>
				(bls12_381.G1.hashToCurve(toU8(msg), { DST: toU8(DST) }) as PointG1),
			expand_message: functionSuite.suiteParams.expand_message,
			expand_len: 48,
		});
	...
	it("generates the correct P1", async () => {
		...
		const api_id = new TextEncoder().encode("BBS_BLS12381G1_XMD:SHA-256_SSWU_RO_");
