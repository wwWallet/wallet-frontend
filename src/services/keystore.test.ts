...
describe("The keystore", () => {
	it("can initialize the key store with a PRF key.", async () => {
		...
		const [wrappedMainKey, wrappingKey, keyInfo] = await keystore.initPrf(mockCredential, prfSalt, "localhost", async () => false);
		const { mainKey, privateData } = await keystore.init(wrappedMainKey, wrappingKey, keyInfo, "p256-pub");
	...
	it("can initialize the key store with a password key.", async () => {
		const [wrappedMainKey, wrappingKey, keyInfo] = await keystore.initPassword("Asdf123!");
		const { mainKey, privateData } = await keystore.init(wrappedMainKey, wrappingKey, keyInfo, "jwk_jcs-pub");
