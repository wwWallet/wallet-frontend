...
import { CurrentSchema, findMergeBase, foldNextEvent, foldOldEventsIntoBaseState, mergeEventHistories, SchemaV1, SchemaV2 } from "./WalletStateSchema";
...
describe("The current WalletStateSchema version", () => {
	...
	it("should successfully apply 'delete_credential' event on a baseState that includes credentials", async () => {
		...
		container = await CurrentSchema.WalletStateOperations.addDeleteCredentialEvent(
			container,
	...
	it("should successfully find the correct point of divergence between two event histories and merge them", async () => {
		...
		let container1 = await CurrentSchema.WalletStateOperations.addNewCredentialEvent(container, "cred1a", "", "");
		container1 = await CurrentSchema.WalletStateOperations.addNewCredentialEvent(container1, "cred1b", "", "");

		let container2 = await CurrentSchema.WalletStateOperations.addDeleteCredentialEvent(
			container,
			(container.events[1] as any).credentialId,
		);
		container2 = await CurrentSchema.WalletStateOperations.addNewCredentialEvent(container2, "cred2x", "", "");
		container2 = await CurrentSchema.WalletStateOperations.addNewCredentialEvent(container2, "cred2y", "", "");
	...
	it("mergeEventHistories de-duplicates new_credential events by credentialId.", async () => {
		let container: CurrentSchema.WalletStateContainer = CurrentSchema.WalletStateOperations.initialWalletStateContainer();
		container = await CurrentSchema.WalletStateOperations.addNewCredentialEvent(container, "cred0", "", "");
		last(container.events).timestampSeconds = 0;

		const container1 = await CurrentSchema.WalletStateOperations.addNewCredentialEvent(container, "cred1a", "", "");
		last(container1.events).timestampSeconds = 1;
		let container2 = await CurrentSchema.WalletStateOperations.addNewCredentialEvent(container, "cred2a", "", "");
		last(container2.events).timestampSeconds = 2;
		container2 = await CurrentSchema.WalletStateOperations.addNewCredentialEvent(container2, "cred2b", "", "");
	...
	it("mergeEventHistories de-duplicates delete_credential events by credentialId.", async () => {
		...
		const container1 = await CurrentSchema.WalletStateOperations.addDeleteCredentialEvent(
			container,
			(container.events[0] as any).credentialId,
		);
		...
		last(container2.events).timestampSeconds = 3;
		container2 = await CurrentSchema.WalletStateOperations.addDeleteCredentialEvent(container2, (container.events[1] as any).credentialId);
	...
	it("mergeEventHistories de-duplicates new_keypair events by kid.", async () => {
		let container: CurrentSchema.WalletStateContainer = CurrentSchema.WalletStateOperations.initialWalletStateContainer();
		container = await CurrentSchema.WalletStateOperations.addNewKeypairEvent(container, "kid0", { did: "did0" } as CredentialKeyPair);
		last(container.events).timestampSeconds = 0;

		const container1 = await CurrentSchema.WalletStateOperations.addNewKeypairEvent(container, "kid1", { did: "did1" } as CredentialKeyPair);
		last(container1.events).timestampSeconds = 1;
		let container2 = await CurrentSchema.WalletStateOperations.addNewKeypairEvent(container, "kid2", { did: "did2" } as CredentialKeyPair);
		last(container2.events).timestampSeconds = 2;
		container2 = await CurrentSchema.WalletStateOperations.addNewKeypairEvent(container2, "kid1", { did: "did3" } as CredentialKeyPair);
		last(container2.events).timestampSeconds = 3;
	...
	it("mergeEventHistories de-duplicates delete_keypair events by kid.", async () => {
		...
		container = await CurrentSchema.WalletStateOperations.addNewKeypairEvent(container, "kid0", { did: "did0" } as CredentialKeyPair);
		last(container.events).timestampSeconds = 0;
		container = await CurrentSchema.WalletStateOperations.addNewKeypairEvent(container, "kid1", { did: "did1" } as CredentialKeyPair);
		last(container.events).timestampSeconds = 1;

		const container1 = await CurrentSchema.WalletStateOperations.addNewKeypairEvent(container, "kid2", { did: "did2" } as CredentialKeyPair);
		last(container1.events).timestampSeconds = 2;
		let container2 = await CurrentSchema.WalletStateOperations.addNewKeypairEvent(container, "kid3", { did: "did3" } as CredentialKeyPair);
		last(container2.events).timestampSeconds = 3;
		container2 = await CurrentSchema.WalletStateOperations.addNewKeypairEvent(container2, "kid2", { did: "did4" } as CredentialKeyPair);
		last(container2.events).timestampSeconds = 4;
	...
	it("mergeEventHistories de-duplicates new_presentation events by presentationId, and keeps the oldest.", async () => {
		...
		const container1 = await CurrentSchema.WalletStateOperations.addNewPresentationEvent(container, 1, "data1", [], 0, "");
		last(container1.events).timestampSeconds = 1;
		let container2 = await CurrentSchema.WalletStateOperations.addNewPresentationEvent(container, 2, "data2a", [], 0, "");
		last(container2.events).timestampSeconds = 2;
	...
	it("mergeEventHistories de-duplicates delete_presentation events by presentationId, and keeps the oldest.", async () => {
		let container: CurrentSchema.WalletStateContainer = CurrentSchema.WalletStateOperations.initialWalletStateContainer();
		container = await CurrentSchema.WalletStateOperations.addNewPresentationEvent(container, 1, "", [], 2, "");
		last(container.events).timestampSeconds = 0;
		container = await CurrentSchema.WalletStateOperations.addNewPresentationEvent(container, 1, "", [], 2, "");
		last(container.events).timestampSeconds = 1;

		const container1 = await CurrentSchema.WalletStateOperations.addDeletePresentationEvent(
			container,
			(container.events[0] as any).presentationId,
		);
		last(container1.events).timestampSeconds = 2;
		let container2 = await CurrentSchema.WalletStateOperations.addDeletePresentationEvent(
			container,
			(container.events[0] as any).presentationId,
		);
		last(container2.events).timestampSeconds = 3;
		container2 = await CurrentSchema.WalletStateOperations.addDeletePresentationEvent(container2, (container.events[1] as any).presentationId);
		last(container2.events).timestampSeconds = 4;
	...
	it("mergeEventHistories overwrites conflicting alter_settings events with the latest one.", async () => {
		...
		let container: CurrentSchema.WalletStateContainer = CurrentSchema.WalletStateOperations.initialWalletStateContainer();
		container = await CurrentSchema.WalletStateOperations.addAlterSettingsEvent(container, { ...defaultSettings, foo: "bar" });
		last(container.events).timestampSeconds = 0;

		const container1 = await CurrentSchema.WalletStateOperations.addAlterSettingsEvent(container, { ...defaultSettings, foo: "boo" });
		last(container1.events).timestampSeconds = 1;
		let container2 = await CurrentSchema.WalletStateOperations.addAlterSettingsEvent(container, { ...defaultSettings, foo: "far" });
		last(container2.events).timestampSeconds = 2;
		container2 = await CurrentSchema.WalletStateOperations.addAlterSettingsEvent(container2, { ...defaultSettings, foo: "zoo" });
		last(container2.events).timestampSeconds = 3;
	...
	it("mergeEventHistories de-duplicates save_credential_issuance_session events by eventId.", async () => {
		let container: CurrentSchema.WalletStateContainer = CurrentSchema.WalletStateOperations.initialWalletStateContainer();
		container = await CurrentSchema.WalletStateOperations.addSaveCredentialIssuanceSessionEvent(container, 0, "iss0", "", "", "");
		last(container.events).timestampSeconds = 0;

		const container1 = await CurrentSchema.WalletStateOperations.addSaveCredentialIssuanceSessionEvent(container, 1, "iss1", "", "", "");
		last(container1.events).timestampSeconds = 1;
		let container2 = await CurrentSchema.WalletStateOperations.addSaveCredentialIssuanceSessionEvent(container, 2, "iss2a", "", "", "");
		last(container2.events).timestampSeconds = 2;
		container2 = await CurrentSchema.WalletStateOperations.addSaveCredentialIssuanceSessionEvent(container2, 3, "iss2b", "", "", "");
		last(container2.events).timestampSeconds = 3;
	...
	it("resolves a fast-forward merge by returning the younger container unchanged.", async () => {
		...
		let container2 = await CurrentSchema.WalletStateOperations.addNewCredentialEvent(container, "cred2", "", "");
	...
	it("correctly merges diverged containers when one container is partially folded.", async () => {
		...
		{
			let container1a = await CurrentSchema.WalletStateOperations.addNewCredentialEvent(container, "cred2a", "", "");
			let container2a = await CurrentSchema.WalletStateOperations.addNewCredentialEvent(
		...
		{
			let container1b = await CurrentSchema.WalletStateOperations.addNewCredentialEvent(
				await foldNextEvent(container),
				"cred2a", "", "");

			let container2b = await CurrentSchema.WalletStateOperations.addNewCredentialEvent(
	...
	it("mergeEventHistories maintains the per-branch order of events.", async () => {
		...
		container = await CurrentSchema.WalletStateOperations.addNewCredentialEvent(container, "cred2", "", "");
		last(container.events).timestampSeconds = 1;
		container = await CurrentSchema.WalletStateOperations.addDeleteCredentialEvent(container, (container.events[0] as any).credentialId);
		...
		let container2 = await CurrentSchema.WalletStateOperations.addDeleteCredentialEvent(container, (container.events[1] as any).credentialId);
