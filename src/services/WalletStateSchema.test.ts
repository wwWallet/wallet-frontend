...
import { calculateEventHash, CurrentSchema, eventHistoryIsConsistent, findMergeBase, foldNextEvent, foldOldEventsIntoBaseState, mergeEventHistories, reparent } from "./WalletStateSchema";
...
describe("The current WalletStateSchema version", () => {
	...
	it("mergeEventHistories overwrites conflicting alter_settings events with the latest one.", async () => {
		let container: CurrentSchema.WalletStateContainer = CurrentSchema.WalletStateOperations.initialWalletStateContainer();
		container = await CurrentSchema.WalletStateOperations.addAlterSettingsEvent(container, { foo: "bar" });
		last(container.events).timestampSeconds = 0;

		const container1 = await CurrentSchema.WalletStateOperations.addAlterSettingsEvent(container, { foo: "boo" });
		last(container1.events).timestampSeconds = 1;
		let container2 = await CurrentSchema.WalletStateOperations.addAlterSettingsEvent(container, { foo: "far" });
		last(container2.events).timestampSeconds = 2;
		container2 = await CurrentSchema.WalletStateOperations.addAlterSettingsEvent(container2, { foo: "zoo" });
		last(container2.events).timestampSeconds = 3;
