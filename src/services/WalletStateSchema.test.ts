...
describe("The current WalletStateSchema version", () => {
	...
	it("resolves a fast-forward merge by returning the younger container unchanged.", async () => {
		...
		const container2 = await addNewCredentialEvent(container, "cred2", "", "");
	...
	it("correctly merges diverged containers when one container is partially folded.", async () => {
		...
		{
			const container1a = await addNewCredentialEvent(container, "cred2a", "", "");
			const container2a = await addNewCredentialEvent(
		...
		{
			const container1b = await addNewCredentialEvent(
				await foldNextEvent(container),
				"cred2a", "", "");

			const container2b = await addNewCredentialEvent(
	...
	it("mergeEventHistories orders events first in ascending order of schemaVersion.", async () => {
		...
		const container1 = await addNewCredentialEvent(container1v1, "cred1b", "", "");
		...
		const container2 = await addNewCredentialEvent(container2v1, "cred2y", "", "");
