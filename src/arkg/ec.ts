...
export function find_y(crv: Curve, x: bigint): NonzeroPoint | null {
...
export function neg(crv: Curve, a: Point): Point {
	if (a === "zero") {
		return "zero";
	} else {
...
export async function curveFromCoseCurveId(crv: number) {
	switch (crv) {
		case 1:
			return curveSecp256r1();
		default:
			throw new Error("Unsupported curve for COSE identifier: " + crv);
	}
}
