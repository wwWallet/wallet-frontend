	...
	useEffect(() => {
		const initialize = async () => {
			...
			try {
				...
				const issuerResponse = isStandAlone ? { data: [] } : await api.getExternalEntity('/issuer/all');
				const trustedCredentialIssuers = issuerResponse.data;

				const userResponse = isStandAlone ? { data: { settings: {} } } : await api.getExternalEntity('/user/session/account-info')
