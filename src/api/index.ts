	...
	>> => {
		...
		try {
			...
			if (keystore) {
				...
				const mergedContainer = await keystore.syncWithRemoteData(remotePrivateData);
				if (mergedContainer) {
					// Successfully merged - update ETag and push merged state to server
					updatePrivateDataEtag(getPrivateDataResponse);
					const updateResp = updatePrivateDataEtag(
						await post('/user/session/private-data', serializePrivateData(mergedContainer)),
					);
				...
				// If merge failed, fall through to legacy behavior
